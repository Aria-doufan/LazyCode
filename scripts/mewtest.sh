#!/usr/bin/env bash
# LazyCode tmux-based E2E smoke test helper.
# Usage:
#   ./scripts/mewtest.sh start              — start LazyCode in tmux
...
SESSION="mewtest"
LAZYCODE_DIR="$(cd "$(dirname "$0")/.." && pwd)"
BINARY="$LAZYCODE_DIR/lazycode"
WIDTH=120
HEIGHT=40
...
start() {
    ...
    tmux new-session -d -s "$SESSION" -x "$WIDTH" -y "$HEIGHT" \
        "cd '$LAZYCODE_DIR' && '$BINARY'"

    local timeout=10
    local elapsed=0
    while [ $elapsed -lt $timeout ]; do
        if tmux capture-pane -t "$SESSION" -p 2>/dev/null | grep -q "Send a message"; then
            echo "LazyCode started successfully"
            return 0
        fi
        sleep 1
        elapsed=$((elapsed + 1))
    done
    echo "ERROR: LazyCode did not start within ${timeout}s"
    capture
    return 1
}
...
wait_response() {
    ...
    while [ $elapsed -lt $timeout ]; do
        ...
        # LazyCode shows "✻ <PastTenseVerb> for X.Xs" when done.
...
stop() {
    ...
    sleep 1
    tmux kill-session -t "$SESSION" 2>/dev/null || true
    echo "LazyCode session stopped"
}

smoke() {
    echo "=== LazyCode Smoke Test ==="

    echo "[1/4] Starting LazyCode..."
    start
...
case "${1:-help}" in
    ...
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  start              Start LazyCode in tmux"
