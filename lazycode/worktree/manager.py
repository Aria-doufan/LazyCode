...
from lazycode.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from lazycode.worktree.models import Worktree, WorktreeSession
from lazycode.worktree.session import load_worktree_session, save_worktree_session
from lazycode.worktree.setup import perform_post_creation_setup
from lazycode.worktree.slug import flatten_slug, validate_slug
...
class WorktreeManager:
    ...
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".lazycode" / "worktrees"
        )
        self._lazycode_dir = Path(repo_root) / ".lazycode"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._lazycode_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._lazycode_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        """处理restore 会话。"""
        session = load_worktree_session(self._lazycode_dir)
        if session is None:
            return None
