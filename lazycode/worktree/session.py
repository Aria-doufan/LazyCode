...
def _session_path(lazycode_dir: Path) -> Path:
    """处理会话 路径。"""
    return lazycode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    """保存worktree 会话。"""
    path = _session_path(lazycode_dir)
...
def load_worktree_session(lazycode_dir: Path) -> WorktreeSession | None:
    """加载worktree 会话。"""
    path = _session_path(lazycode_dir)
    if not path.exists():
        return None
