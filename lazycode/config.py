...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".lazycode" / "config.yaml",
        cwd / ".lazycode" / "config.yaml",
        cwd / ".lazycode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .lazycode/config.yaml "
            "in project or ~/.lazycode/config.yaml"
        )
    return merged
