...
def is_coordinator_mode(enable_flag: bool = False) -> bool:
    """判断coordinator mode是否成立。"""
    if not enable_flag:
        return False
    val = os.environ.get("LAZYCODE_COORDINATOR_MODE", "").lower()
...
def match_session_mode(
    ...
    if session_is_coordinator:
        os.environ["LAZYCODE_COORDINATOR_MODE"] = "1"
    else:
...
def get_coordinator_system_prompt(agent_catalog: list[tuple[str, str]] | None = None) -> str:
    ...
    return """You are LazyCode, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
def get_coordinator_user_context(
    ...
    """获取coordinator user 上下文。"""
    if worker_tools is None:
        from lazycode.agents.tool_filter import IN_PROCESS_TEAMMATE_ALLOWED_TOOLS
