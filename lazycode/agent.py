...
@dataclass
class StreamText:
    """封装Stream Text相关状态和行为。"""
    text: str


@dataclass
class ThinkingText:
    """封装Thinking Text相关状态和行为。"""
    text: str
...
@dataclass
class TurnComplete:
    """封装Turn Complete相关状态和行为。"""
    turn: int


@dataclass
class LoopComplete:
    """封装Loop Complete相关状态和行为。"""
    total_turns: int
...
class StreamCollector:
    """封装Stream Collector相关状态和行为。"""
    def __init__(self) -> None:
        """初始化Stream Collector实例。"""
...
class Agent:
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".lazycode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from lazycode.permissions.rules import Rule, extract_content
