...
from lazycode.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
...
from lazycode.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from lazycode.memory.session import (
    RecordType,
    ResumeResult,
    Session,
    SessionManager,
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        lazycode_md = tmp_path / "LAZYCODE.md"
        lazycode_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        root_md = tmp_path / "LAZYCODE.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".lazycode"
        dotdir.mkdir()
        dot_md = dotdir / "LAZYCODE.md"
...
class TestMemoryManager:
    ...
    def test_load_merges_user_and_project(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        user_mem = fake_home / ".lazycode" / "memories.md"
        user_mem.parent.mkdir(parents=True)
        user_mem.write_text("### 用户偏好\n- prefer spaces", encoding="utf-8")

        project_mem = tmp_path / "project" / ".lazycode" / "memories.md"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "lazycodeMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "lazycodeMd" in conv.history[0].content
...
# =========================================================================
# I. Memory 提取 prompt 构建
# =========================================================================

class TestMemoryExtraction:
    def test_extraction_prompt_contains_categories(self, tmp_path: Path) -> None:
        from lazycode.memory.auto_memory import MEMORY_EXTRACTION_PROMPT
