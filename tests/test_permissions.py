...
from lazycode.tools.base import StreamEnd, StreamEvent, TextDelta, ToolCallComplete

# ===========================================================================
# 第 1 层：DangerousCommandDetector
# ===========================================================================
...
class TestPathSandbox:
    ...
    def test_temp_dir_allowed(self) -> None:
        tmp = Path(tempfile.gettempdir()) / "lazycode_test.txt"
...
class TestRuleEngine:
    ...
    def test_append_local_rule(self) -> None:
        tmpdir = Path(tempfile.mkdtemp())
        local_path = tmpdir / ".lazycode" / "permissions.local.yaml"
...
# ===========================================================================
# 第 5 层（集成）：PermissionChecker — 五层组合
# ===========================================================================
...
async def test_e2e_rule_allows_git():
    """允许 git 命令的规则会让它们无需 HITL 即通过。"""
    tmpdir = Path(tempfile.mkdtemp())
    rules_file = tmpdir / ".lazycode" / "permissions.yaml"
...
async def test_e2e_default_mode_write_triggers_ask():
    """在 default 模式下，写入工具会产生 ASK → PermissionRequest event。"""
    tmpdir = Path(tempfile.mkdtemp())
    client = MockLLMClient([
        [
