...
"""Agent Loop 集成测试 — 以编程方式验证 checklist 项。"""
...
async def test_multi_step_autonomous():
    """Agent 执行 WriteFile 后再执行 ReadFile，然后停止 — 端到端多步骤。"""
    client = MockLLMClient([
        # 第 1 轮：WriteFile
        [
            TextDelta("Creating file."),
            ToolCallComplete("t1", "WriteFile", {"file_path": "/tmp/lazycode_test_hello.txt", "content": "Hello World"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        # 第 2 轮：ReadFile 用于验证
        [
            TextDelta("Verifying content."),
            ToolCallComplete("t2", "ReadFile", {"file_path": "/tmp/lazycode_test_hello.txt"}),
...
async def test_stop_max_iterations():
    """Agent 到达 max_iterations 后停止。"""
    # 每个响应都有一个 tool call，因此循环不会自然结束
    responses = []
    for i in range(5):
        responses.append([
            TextDelta(f"Step {i}"),
...
async def test_stop_cancel():
    """Agent 在 CancelledError 时干净停止。"""

    class SlowMockClient(LLMClient):
        """在事件之间 sleep 以允许取消的 mock 客户端。"""
...
async def test_message_splicing():
    """Assistant 消息包含 text + 多个 tool_uses；tool_results 会被打包。"""
    client = MockLLMClient([
        # 第 1 轮：一次响应中有两个 tool call
        [
    ...
    assert len(tool_results_msg["content"]) == 2  # 2 个 tool_results
...
async def test_plan_mode_denied_tool_returns_error():
    """在 Plan 模式下，写入工具会被 permission checker 拒绝。"""
    ...
    await asyncio.wait_for(collect_events(), timeout=1)
...
def test_partition_tool_calls():
    """Partition 会将可安全并发的调用分组到一起。"""
    from lazycode.tools.base import ToolCallComplete
