...
"""Hook 系统测试 — events、conditions、executors、engine、loader 和 agent 集成。"""
...
class TestCommandExecutor:
    ...
    async def test_cancel_kills_running_command(self, monkeypatch: pytest.MonkeyPatch):
        ...
        task = asyncio.create_task(
            executors.execute_command(Action(type="command", command="slow"), HookContext())
        )
...
class TestAgentHookIntegration:
    """测试 pre_tool_use reject 会导致跳过 tool call。"""

    @pytest.mark.asyncio
    async def test_pre_tool_use_reject_skips_tool(self):
        from lazycode.agent import Agent, ToolResultEvent
